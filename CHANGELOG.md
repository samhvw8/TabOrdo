...
- **Requires Chrome 138 or later** — the version from June 2025 that brought on-device AI to extensions. Messages about older Chrome versions are gone, and "Save to Reading List" is always in the toolbar icon menu
- **Auto-discard also unloads tabs Chrome has paused** — a paused tab still holds its memory, and auto-discard used to skip it. One rule now decides for auto-discard, "Discard inactive tabs" and `/freeze`: never the active tab, a pinned tab, a tab playing sound, or one already unloaded
- **The volume slider is gone from dashboard tab cards** — without access to every page it only worked on the tab you had just opened TabOrdo on, and silently did nothing on the rest. Mute stays, and `/vol` still sets the volume and says which tabs it could not reach
...
- **Auto-sort no longer rearranges a window that is already in order** — every tab that finished loading re-laid the whole window, about 25 browser calls each time and over 120 in a window of 1000 tabs, even when nothing was out of place. An already sorted window now costs a couple of calls, and a single stray tab costs a few more. With locked groups, a sort used to shuffle them into alphabetical order and then move them back; they are now put straight into their slots, so the strip no longer churns while you watch
- **Opening many links at once is cheaper** — TabOrdo records where they came from in one write instead of one per link, and a new tab joining an existing domain group no longer fetches every tab in the window
- **Undo no longer copies its whole history on every action** — each undo step is stored on its own, so closing a single tab with a full undo history writes a few hundred bytes instead of rewriting over a megabyte, and opening the popup only checks which steps exist rather than reading the whole history. Undoing a sort or shuffle now moves tabs back in a handful of calls instead of one per tab, and groups the action never touched keep their id and collapsed state instead of being rebuilt
- **Faster typing in the search box** — one-letter and accented queries skip the fuzzy matcher, which was doing the most work for no extra results; with accents, it also stops listing every tab that merely contains an "h" for `hư`. The search index is built after the popup appears rather than before, and `@` views keep theirs between keystrokes. `/w` and `/g` filter the tabs the popup already has instead of asking Chrome on every key, `/rl` and `/rc` load their list once per visit, and `/b` and `/h` wait until you pause typing before searching (Enter still opens the first match straight away). The Rules, Pins and Settings panels load when first opened, which trims about 11% of the popup's startup script
- **Group, Regroup, /collapse, /freeze and /readlater send less** — groups already collapsed or expanded as they should be are left alone, grouping no longer fetches every tab twice, and discarding or saving many tabs sends the requests together instead of one after another
- **Auto-sort stops shuffling tabs while they load** — every page that finished loading started a full sort of its window, and when several finished together those sorts ran at the same time and undid each other's moves. Opening 200 tabs made Chrome move tabs 1,262 times and regroup them 1,214 times; it now takes about 20 moves and 200 regroups. A window is sorted once, a quarter of a second after its last page finishes loading, and a page that loaded before auto-group put it in its group now gets sorted into place too
- **Typing in the palette is smoother with many tabs** — the highlighted row no longer makes the popup lay itself out again on every keystroke, and a view opened with nothing typed, such as a bare `@`, lists its first 100 tabs and a "Showing 100 of 817, type to narrow" line instead of every tab at once. At 1,000 tabs the slowest keystroke went from 89 ms to 43 ms, and on a slower computer from 369 ms to 138 ms
- **Half the download** — TabOrdo is about 720 KB instead of 1.5 MB. The code is now minified, and the list of web domains it uses to name groups drops the half that holds hosting services like `github.io`, which TabOrdo never consulted. The background script, which Chrome reloads every time it wakes it, is less than half its old size
...
- **AI grouping gets its instructions** — Chrome silently dropped the instructions TabOrdo gave Gemini Nano, so the model never learned what answer to give, which is behind many of the "AI found no groups" results. It now gets them, along with a fixed shape for its answer. An answer that fails or cannot be read now shows an error instead of "no groups", and with more tabs than the on-device model can take in, it groups as many as fit and says how many it left alone
- **The side panel fills its space** — it was drawn as the popup's 450 by 600 box, cutting off its right edge in a narrow panel and leaving empty space in a wide or tall one
- **Sorting one group keeps it in place** — it used to carry the whole group to the end of the window
- **Group locks land where you put them** — a group locked to a position on its right now lands there instead of one group further, and a group of several tabs no longer comes apart on the way. Groups locked to the same position keep a steady order instead of swapping on every sort, and sorting a window that is already in order moves nothing. This applies to sorting, auto-sort, Group, Regroup, `/lockgroup` and `/movegroup`. A group locked to the last position now sits after the other groups and before the ungrouped tabs
...
- **The automation activity list keeps every entry** — when several tabs were grouped at once, their entries overwrote each other and only the last one was kept
...
- **Undo after a partly failed close no longer creates duplicates** — a bulk close records what it is about to close before it closes anything, so the record survives the popup being torn down mid-action. When Chrome then refused one of those tabs — a tab being dragged, for instance — Ctrl+Z reopened every tab in the record, the refused one included, and you ended up with two copies of a tab you had tried to lose. Undo now restores only the tabs that are actually gone. This affected `/closeleft`, `/closeright`, `/closesite`, `/closeold` and `/dedup` alike: 0.7.2 described `/dedup` as having been brought in line with how the other commands closed, but those commands handed Chrome their whole list in one call, which stops at the first tab it cannot close and leaves the rest open, with the error message inviting exactly the undo that then doubled them
- **Every close in TabOrdo now goes through one path** — one function snapshots for undo, closes tab by tab so a stale entry cannot stop the ones after it, and reports the result; a test fails the build if a second way to close a tab appears. That is what makes the fixes above hold for every command instead of the one that was last reported, and it caught one that had already slipped: the dashboard's **Archive** button for selected tabs took no undo snapshot at all, so Ctrl+Z after it restored whatever unrelated action was on the stack. Both selection buttons now run the same handlers as `/close` and `/archive`
- **A tab Chrome would not close is reported again** — 0.7.2 counted only the tabs it managed to close and said "No duplicates found" when Chrome had refused all of them, hiding the one message you could act on. A tab that had already gone by itself still counts as done, since that is what you asked for; a tab that is still there after the close is now an error, shown after the rest of the batch has been closed
- **Status counts are what happened** — `/close` and the dashboard's Close button reported the number of tabs they set out to close rather than the number that are no longer open
...
- **Gather a reading branch — `/branch` and `/branchup`** — open a listing page, middle-click your way through it, then keep clicking from the articles themselves, and you end up with thirty tabs whose only relationship is that one opened another. `/branch` puts the active tab and everything opened from it, at any depth, into a group of its own; members already in some other group are taken out of it, and members that drifted to another window are pulled back first. `/branchup` runs the same gather one level up, so from an article you get the listing page, its other articles, and everything those opened. Run it again and it climbs: it walks past the ancestors the previous run already pulled into your group rather than rebuilding the group you have. Trailing text names the group (`/branch Morning read`); with none, the group takes the root tab's title, or its hostname when that title is too long for a group strip. The group always forms in the window you are looking at, never the root's — for `/branchup` the root can be an ancestor in another window, and building it there would drag your active tab off-screen. Shared groups and groups your ignore list protects by name are left whole. The status line accounts for everything the command declined to do, and keeps the reasons apart: members left in a protected group, members Chrome would not group because they are pinned, and a root that could not join at all. When there is nothing left above to gather, `/branchup` says so rather than claiming the tab was never opened from another — its parent may be sitting right beside it in the group already. Both are available as dashboard tiles too, and they alt-click to each other, so one starred tile reaches both. Note the branch is a snapshot, not a container: with auto-sort on, Chrome tabs opened from a branch member afterwards are grouped by domain as usual, and the group's internal order is re-sorted

  Chrome's own `openerTabId` is thin: it vanishes the moment the opener closes, never survives a restart, and Chrome's tab strip forgets *every* opener the moment any tab navigates by something other than a link click — typing a URL anywhere wipes the lot. TabOrdo records the link when the tab is created and keeps its own map in session storage, which buys the case that actually breaks a reading session — an *intermediate* tab closing. Close an article and its sub-links re-parent to the listing page above them, so gathering from that page still catches them. Closing a *root* is the one case the map cannot rescue: with no grandparent to inherit, its children become roots and stop being each other's siblings

- **Run `/branch` again and it adds, rather than rebuilds** — Chrome drops a tab opened from a grouped tab into that group by itself, so a second run is mostly about strays: tabs that drifted to another window, or were pulled into some other group. When the root's group holds nothing but branch members, the run folds the newcomers into it and reports `Added 3 tab(s)`; the group keeps its title, colour, collapse state and whatever identity Chrome's saved groups gave it, and a trailing name renames it in place. A branch that is already whole says so and leaves both the strip and the undo stack alone. A group with a stranger in it — the listing page auto-grouped with an unrelated tab from the same site — is not this branch, and the command still builds a group of its own
- **`@b` — see the branch before gathering it** — the active tab's branch as an outline: the root, then everything opened from it, indented one level per hop, in tree order. It is the view of exactly what `/branch` would gather, and the place to look when it grabbed (or missed) a tab you did not expect. Being a result list, it also takes the list's own actions, so a rabbit hole can be closed from it without grouping it first
- **`/parent` — back to the tab that opened this one** — the navigation half of lineage. Chrome does this itself when you close a tab, but only while its own opener link survives, which it rarely does; this reads TabOrdo's map, so it works after the middle of the chain has closed and across windows

### Bug Fixes

- **`/dedup` no longer closes a tab you pinned to a position** — Chrome's own pin has always survived dedup, but a `/pin` never did: survivors within a duplicate set were picked by last-accessed time, so the copy you had deliberately fixed to a slot in its group lost to any copy you had touched more recently, and the pin entry was left pointing at a tab that no longer existed. Position pins now count as a keep alongside Chrome's, resolved the way the rest of the pin code resolves them — tab id first, then URL within the pin's own group. One entry protects one tab, so a second copy of the same page sitting inside that group is still a duplicate and still closes

- **Fix `/branch` gathering a tab you opened with Ctrl+T** — Chrome hands a new-tab page the tab you were looking at as its opener, and keeps that link through the first URL typed into it, as a quick-lookup heuristic. TabOrdo recorded it as lineage, so standing on a listing page, pressing Ctrl+T and typing an unrelated address made that tab a member of the listing page's branch. New-tab pages are now recorded as explicit roots, and that also stops Chrome's live value from re-introducing the link when the two are merged
