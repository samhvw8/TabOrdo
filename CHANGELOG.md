...
- **Undo no longer copies its whole history on every action** — each undo step is stored on its own, so closing a single tab with a full undo history writes a few hundred bytes instead of rewriting over a megabyte, and opening the popup reads only the step names rather than the whole history. Undoing a sort or shuffle now moves tabs back in a handful of calls instead of one per tab, and groups the action never touched keep their id and collapsed state instead of being rebuilt
