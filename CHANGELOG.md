...
- **Tab events stop re-reading your locks** — the locked tab and group lists are kept in memory and refreshed when they change, instead of being read from storage on every title or URL change of every tab. Opening many links at once records where they came from in one write instead of one per link, and a new tab joining an existing domain group no longer fetches every tab in the window
