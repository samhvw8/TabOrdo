...
- A module that touches `chrome` at import time needs the stub first. `rules.ts` registers `storage.onChanged` at module scope, so `rules-cache.test.ts` installs, calls `vi.resetModules()`, and re-imports per test.[^rules-cache-test]
